<?php
//$target_dir = "uploads/";
$target_dir = "../student/qb/";
$target_dir = $target_dir . str_replace(' ','', basename( $_FILES["uploadFile"]["name"]));
$uploadOk=1;
...
if(isset($_FILES['uploadFile'])) {
    ...
    if($_FILES['uploadFile']['error'] == 0) {
        ...
        $query = "
            INSERT INTO `qb` (
                `course`,`sem`,`paper_code`,`name`, `mime`, `size`, `data`, `created`
            )
            VALUES (
                '{$course}','{$sem}','{$code}','{$name}', '{$mime}', {$size}, '{$data}', '{$date}'
            )";
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["uploadFile"]["tmp_name"], $target_dir)) {
        echo "The file ". str_replace(' ','',basename( $_FILES["uploadFile"]["name"])). " has been uploaded.";
    } else {
