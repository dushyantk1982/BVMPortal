...
<body><div class="style2" id="Layer4" style="border:solid;border-color:#003300;">
                        <form action="st_list.php?id=st_list" method="post" onsubmit="MM_validateForm('session','','R');return document.MM_returnValue" >
...
<?php
	...
	mysql_select_db("bvmgroup_bvmclg")or die(mysql_error());					 
		...
		$res=mysql_query("select * from student_reg where status='active'",$cn);
		echo "<table width='500' border='1'><tr><td>S.No.</td><td>File No</td><td>Name</td></tr>";
		$i=1;
	while($row=mysql_fetch_array($res))
	{
		if(strcmp($course,$row['course'])==0 && strcmp($sem,$row['sem'])==0)
		{echo "<tr><td>".$i."</td><td>".$row['file_no']."</td><td>".$row['st_name']."</td></tr>";
		$i=$i+1;
		}
