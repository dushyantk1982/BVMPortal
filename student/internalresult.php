						...
						include('dbcon.php');
							//$conn=mysql_connect("localhost:3306","bvmgroup","PWD@bvm1");
						 		if(!$conn)
						 			{die('Could not connect:'.mysql_error());}
						 		//mysql_select_db('bvmclg');
								$col=mysql_query("select * from internal_record where file_no='". $fno ."' and sem='". $sem ."' and course='". $course ."' and internal='". $int ."' and session='".$ses."' order by paper_code ASC",$conn);
						 //if(!$col)
						 //{die('Could not connect'.mysql_error());}
						 while($row=mysql_fetch_array($col,MYSQL_ASSOC))
						 {
